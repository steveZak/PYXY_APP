...
export default class Profile extends React.Component {
  // add hometown, favourite cities, later -> visited places
  state = {
    uid: null,
    combinedTrips: [],
    loading: false,
  ...
  async removeTrip() {
    await fetch(SERVER_ADDR+"/remove_trip?city_id="+this.state.removeCity+"&trip_id="+this.state.removeTrip, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }}).then(response => response.json()).then(responseJson => {if(responseJson.ok){
        var trips = this.state.combinedTrips;
        trips.splice(this.state.removeIdx, 1);
        this.setState({removeModal: false, combinedTrips: trips})
      }});
  }
  ...
  getRemoveModal() {
    return(<View style={{backgroundColor: 'white',
        ...
        <View style={{height: height*0.05, width: width*0.6}}>
          <Text style={{fontSize: width*0.05,
                        fontFamily: 'Montserrat-Regular',
                        textAlign: 'center'}}>{"Are you sure?"}</Text>
        </View>
        <TouchableOpacity onPress={()=>this.removeTrip()}>
  ...
  async unsaveTrip() {
    firebase.firestore().collection("users").doc(this.state.uid).update({saved_trips: firebase.firestore.FieldValue.arrayRemove({"trip_id": this.state.removeTrip, "city_id": this.state.removeCity, "privacy": this.state.combinedTrips[this.state.removeIdx].privacy})});
    var trips = this.state.combinedTrips;
    trips.splice(this.state.removeIdx, 1);
    this.setState({unsaveModal: false, combinedTrips: trips})
  }
  ...
  getUnsaveModal() {
    return(<View style={{backgroundColor: 'white',
        ...
        <TouchableOpacity onPress={()=>this.unsaveTrip()}>
          <View style={{height: height*0.05,
                        width: width*0.45,
                        justifyContent: 'center',
                        backgroundColor: 'red',
  ...
  async changePrivacy() { // changes privacy for trips created by you, creates a new trip if created by others
    var place_ids = ""
    for (var i in this.state.combinedTrips[this.state.removeIdx].places) {
      place_ids= place_ids+"&place_ids[]="+this.state.combinedTrips[this.state.removeIdx].places[i].place_id
    }
    await fetch(SERVER_ADDR+"/edit_trip?city_id="+
                this.state.removeCity+"&uid="+
                this.state.uid+"&trip_id="+
                this.state.removeTrip+place_ids+"&name="+
                encodeURIComponent(this.state.combinedTrips[this.state.removeIdx].label)+"&description="+
                encodeURIComponent(this.state.combinedTrips[this.state.removeIdx].description)+"&privacy="+
                this.state.privacyChange, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }}).then(response => response.json()).then(responseJson => {
        var trips = this.state.combinedTrips;
        trips[this.state.removeIdx].privacy = this.state.privacyChange;
        this.setState({privacyModal: false, combinedTrips: trips})
      });
  }

  async changePrivacySaved() { // changes privacy for saved trips
    firebase.firestore().collection("users").doc(this.state.uid).update({saved_trips: firebase.firestore.FieldValue.arrayRemove({"trip_id": this.state.removeTrip, "city_id": this.state.removeCity, "privacy": this.state.combinedTrips[this.state.removeIdx].privacy})});
    firebase.firestore().collection("users").doc(this.state.uid).update({saved_trips: firebase.firestore.FieldValue.arrayUnion({"trip_id": this.state.removeTrip, "city_id": this.state.removeCity, "privacy": this.state.privacyChange})});
    var trips = this.state.combinedTrips;
    trips[this.state.removeIdx].privacy = this.state.privacyChange;
    this.setState({privacyModal: false, combinedTrips: trips})
  }
    ...
    return(<View style={{backgroundColor: 'white',
        ...
        <View style={{height: height*0.05, width: width*0.75}}>
          <Text style={{fontSize: width*0.045,
                        fontFamily: 'Montserrat-Regular',
                        textAlign: 'center'}}>{"Select who can view this trip"}</Text>
        </View>
        <View style={{height: height*0.15,
                ...
                <TouchableOpacity style={{flexDirection: 'row', alignItems: 'center', marginBottom: height*0.005}} onPress={() => this.setState({privacyChange: 'private'})}>
                  <View style={{borderColor: PRIMARY,
                                ...
                                overflow: 'hidden',
                                width: height*0.03,
                                backgroundColor: this.state.privacyChange=='private'?PRIMARY:'white'}}>
                  </View>
                ...
                <TouchableOpacity style={{flexDirection: 'row', alignItems: 'center', marginBottom: height*0.005}} onPress={() => this.setState({privacyChange: 'limited'})}>
                  <View style={{borderColor: PRIMARY,
                                borderWidth: 1,
                                aspectRatio: 1,
                                borderRadius: 1000,
                                overflow: 'hidden',
                                width: height*0.03,
                                backgroundColor: this.state.privacyChange=='limited'?PRIMARY:'white'}}>
                  </View>
                ...
                <TouchableOpacity style={{flexDirection: 'row', alignItems: 'center'}} onPress={() => this.setState({privacyChange: 'public'})}>
                  <View style={{borderColor: PRIMARY,
                                borderWidth: 1,
                                aspectRatio: 1,
                                borderRadius: 1000,
                                overflow: 'hidden',
                                width: height*0.03,
                                backgroundColor: this.state.privacyChange=='public'?PRIMARY:'white'}}>
                  </View>
          ...
          <TouchableOpacity onPress={()=>{this.state.combinedTrips[this.state.removeIdx].type=='saved'?this.changePrivacySaved():this.changePrivacy()}}>
          <View style={{height: height*0.05,
                        width: width*0.45,
                        justifyContent: 'center',
                        backgroundColor: 'red',
  ...
  // async getPreferences(cats) {
  //   var topCats = [];
  //   for(var i = 0; i<3; i++) {
  //     const maxIdx = cats.map((x, i) => [x, i]).reduce((r, a) => (a[0] > r[0] ? a : r))[1];
  //     topCats.push({cat: avgCats["cat_names"][maxIdx], val: 100*GetZPercent(cats[maxIdx])});
  //     cats[maxIdx] = -100;
  //   }
  //   this.setState({zcats: topCats})
  // }

  getSettings() {
    return(<View style={{height: height*0.3, width: width*0.8,
      alignSelf: 'center', backgroundColor: 'white', padding: normalize(5),
      borderRadius: normalize(10)}}>
      {/* {this.state.zcats==null?<View/>:
      <View style={{height: height*0.24}}>
        <View style={{flexDirection: 'row'}}>
          <Text style={{width: width, height: height*0.08, fontFamily: 'Montserrat-Light', alignSelf: 'center', fontSize: width*0.04}}>{this.state.zcats[0].cat +": "+Math.round(this.state.zcats[0].val)+ "th percentile"}</Text>
        </View>
        <View style={{flexDirection: 'row'}}>
          <Text style={{width: width, height: height*0.08, fontFamily: 'Montserrat-Light', alignSelf: 'center', fontSize: width*0.04}}>{this.state.zcats[1].cat +": "+Math.round(this.state.zcats[1].val)+ "th percentile"}</Text>
        </View>
        <View style={{flexDirection: 'row'}}>
          <Text style={{width: width, height: height*0.08, fontFamily: 'Montserrat-Light', alignSelf: 'center', fontSize: width*0.04}}>{this.state.zcats[2].cat +": "+Math.round(this.state.zcats[2].val)+ "th percentile"}</Text>
        </View>
      </View>} */}
      <View style={{height: height*0.1, marginTop: height*0.05}}>
          <View style={{width: width*0.75, alignSelf: 'center'}}>
              <SwipeButton
                  railBackgroundColor={'white'}
                  railFillBackgroundColor={PRIMARY}
                  railBorderColor={PRIMARY}
                  railFillBorderColor={'transparent'}
                  thumbIconBorderColor={'transparent'}
                  thumbIconBackgroundColor={PRIMARY}
                  thumbIconComponent={Next}
                  swipeSuccessThreshold={75}
                  title={"Swipe to clear preferences"}
                  titleStyles={{fontFamily: 'Montserrat-Light', fontSize: normalize(14)}}
                  onSwipeSuccess={() => this.clearData()}
              />
            </View>
        </View>
        <View style={{height: height*0.1}}>
          <View style={{width: width*0.75, alignSelf: 'center'}}>
            <SwipeButton
                ...
                titleStyles={{fontFamily: 'Montserrat-Light', fontSize: normalize(14)}}
                onSwipeSuccess={()=>this.logOut()}
            />
          </View>
        </View>
          <SpecialIcon
            name='close-circle-outline'
            style={{position: 'absolute', right: width*0.01, marginTop: width*0.01}}
            size={width*0.06}
            color={SECONDARY}
  ...
  render() {
    ...
    return (<SafeAreaView edges={['bottom', 'left', 'right']} style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <Modal onBackdropPress={() => this.setState({settings: false})}
              deviceWidth={width} deviceHeight={height}
              isVisible={this.state.settings}>
          {this.getSettings()}
        </Modal>
        <Modal onBackdropPress={() => this.setState({removeModal: false})}
              deviceWidth={width} deviceHeight={height}
              isVisible={this.state.removeModal}>
          {this.getRemoveModal()}
        </Modal>
        <Modal onBackdropPress={() => this.setState({unsaveModal: false})}
              deviceWidth={width} deviceHeight={height}
              isVisible={this.state.unsaveModal}>
          {this.getUnsaveModal()}
        </Modal>
        <Modal onBackdropPress={() => this.setState({privacyModal: false})}
              deviceWidth={width} deviceHeight={height}
              isVisible={this.state.privacyModal}>
          {this.getPrivacyModal()}
        </Modal>
      </SafeAreaView>);
    }
  }
...
class Next extends React.Component {
  render() {
    return(
      <SpecialIcon name={'chevron-right'} size={normalize(40)} color={SECONDARY}/>
    );
  }
}
